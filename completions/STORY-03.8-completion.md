...
- [x] `scripts/install.sh` created and executable -- `ls -la scripts/install.sh` shows `-rwxr-xr-x`
...
- [x] `scripts/post-setup.sh` created -- runs rebuild steps (graphify, wiki lint) automatically after user adds content -- Script created with wiki lint execution and graphify status check
...
Usage: ./scripts/install.sh [--skip-rtk] [--skip-graphify] [--skip-lasso]

# Test idempotency
$ ./scripts/install.sh --skip-graphify
[OK] Installation complete!
# Second run also exits 0 with "already installed" messages

# Test post-setup script
$ ./scripts/post-setup.sh --check-only
...
- `scripts/install.sh` -- New file: 443 lines, environment bootstrap script with prerequisite checks, plugin installation, and summary reporting
- `scripts/post-setup.sh` -- New file: 219 lines, post-setup rebuild script with wiki lint, graphify check, and canonical-facts verification

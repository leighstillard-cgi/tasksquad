...
func (w *Watcher) Start() error {
	...
	filesToWatch := []string{
		w.dataFile("dispatch-log.md"),
		filepath.Join(w.worklogPath, "backlog.md"),
	}

	// Watch directories
	dirsToWatch := []string{
		w.dataDir("completions"),
		w.dataDir("escalations"),
		w.dataDir("dispatches"),
		w.dataDir("session-logs"),
	}
...
func (w *Watcher) dataDir(name string) string {
	consolidated := filepath.Join(w.worklogPath, "data", name)
	if _, err := os.Stat(consolidated); err == nil {
		return consolidated
	}
	return filepath.Join(w.worklogPath, name)
}

func (w *Watcher) dataFile(name string) string {
	consolidated := filepath.Join(w.worklogPath, "data", name)
	if _, err := os.Stat(consolidated); err == nil {
		return consolidated
	}

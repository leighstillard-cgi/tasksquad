...
async function readDispatchResponse(response) {
...
async function submitDispatch(event) {
    ...
    try {
        ...
        const result = await readDispatchResponse(response);

        if (response.ok) {
            let message = `Dispatch created: ${result.path}`;
            if (result.worker_status === 'started') {
                message += `; worker started (PID ${result.worker_pid})`;
            } else if (result.worker_status === 'failed') {
                message += `; worker failed to start: ${result.worker_error || 'unknown error'}`;
            }
            setDispatchResult(message, result.worker_status === 'failed' ? 'error' : 'success');
            form.reset();
            try {
...
document.getElementById('dispatchForm')?.addEventListener('invalid', function(event) {
    const label = event.target.labels?.[0]?.textContent || 'Required field';
    setDispatchResult(`${label} is required.`, 'error');
}, true);

...
| Wiki index page | Y (data-worklog) | Y | N | already-ported | STORY-03.1: `data/wiki/wiki.md` index. |
...
| Dispatch file tracking | Y | Y | N | already-ported | Writes to `data/dispatches/`. |
| Session logging | Y | Y | N | already-ported | Writes to `data/session-logs/`. |
...
| Pattern | In Reference? | In TaskSquad? | Gap? | Recommendation | Rationale |
|---------|---------------|---------------|------|----------------|-----------|
| Escalation file creation | Y | Y | N | already-ported | Writes to `data/escalations/`. |
...
- Wiki index page (data/wiki/wiki.md)
...
- claude-mem documentation (`data/project/tooling.md`)

...
- `core/templates/story-completion.md` — the format you'll write your completion report in
...
├── Reads: data/dispatch-log.md, data/completions/, data/escalations/, backlog.md
├── Queries: K8s API for pod status + log streaming
├── Writes: data/dispatches/ (manual dispatch), data/observer-log.md (prompt audit)
...
**Active work panel:**
- Parse `data/dispatch-log.md` for rows where status = `dispatched`
...
**Completion feed:**
- List files in `data/completions/` (excluding `archive/`)
...
**Escalation panel:**
- List files in `data/escalations/`
...
- Input: target repo
- Submit: writes a dispatch file to `data/dispatches/` using the template format
- Updates `data/dispatch-log.md` and `backlog.md`
...
- On submit: append to `data/observer-log.md` with timestamp, author (from session), target pod, mode, prompt text
...
- Active work panel shows dispatch status from `data/dispatch-log.md` only (no pod status)
- Live monitor is disabled (no pods to stream from)
- Observer prompt UI still works (logs to `data/observer-log.md`, delivery not attempted)
...
| Active work panel shows dispatched stories | Manually add a row to data/dispatch-log.md, verify it appears |
| Completion feed shows reports | Drop a test completion file in data/completions/, verify it appears |
...
| Manual dispatch creates dispatch file | Submit form, verify file in data/dispatches/ and data/dispatch-log.md updated |
| Live monitor streams logs | Start a test pod, verify output appears in terminal pane |
| Observer /btw prompt logged | Send a /btw prompt, verify it appears in data/observer-log.md with correct mode |
| Observer direct prompt logged | Send a direct prompt, verify it appears in data/observer-log.md with correct mode |
...
- [ ] Implement markdown parser: read data/dispatch-log.md, backlog.md, completion reports, escalations
- [ ] Build active work panel: parse dispatch log, query K8s pod status, render HTML
- [ ] Build completion feed: list data/completions/, parse frontmatter, render HTML
- [ ] Build escalation panel: list data/escalations/, render with badge count, resolve action
...
- [ ] Build observer prompt input: text field, mode toggle, writes to observer-input.txt + data/observer-log.md
